...
- [ ] T2: ToolCallLimitHook 已挂载（hooks 非空），连续失败 3 次熔断并输出失败摘要
  - ⚠ 2026-09-02 T8 回放实测：熔断部分**失效**（ops 工具 dict 无 `content` 键 → strands 一律包成 `status=success`；`ToolResult` 是 TypedDict，`getattr` 取 status 恒为 success）。仅总上限 50 有效。以 `xfail(strict=True)` 锁定，修复轮摘标记并核销本项。
...
- [x] T8: 5 个回放场景集沉淀并接入 pytest
  - 2026-09-02 实测：s1 模式连续性 / s2 总上限熔断 / s3 todo 长任务 / s4 验证回环 / s5 记忆召回（+ s2b 连续失败熔断，xfail 锁定已知缺陷）。`pytest strands_backend/tests/replay -m replay` → **5 passed / 1 xfailed（5.4s）**，无网络无真实 SSH。
