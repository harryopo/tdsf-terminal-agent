...
/**
 * 按 `===NAME===` 标记把合并采集命令的输出切成各段，标记之前的内容一律丢弃。
 *
 * 标记按**出现位置**匹配，不要求独占行首：部分服务器连非交互 exec 都会先往
 * stdout 吐一段欢迎横幅，横幅末尾常常没有换行，第一条标记就会被粘在横幅尾巴上
 * （`…Last login: …===STAT===`）。按行首匹配会把整段 STAT 丢掉，面板因此报
 * 「解析采集数据失败」。
 */
export function splitSections(output: string): Map<string, string> {
  const sections = new Map<string, string>();
  const parts = output.split(/===([A-Za-z_]\w*)===/);
  // split 带一个捕获组 → [前缀, 段名1, 段体1, 段名2, 段体2, …]
