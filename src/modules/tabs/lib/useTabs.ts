...
export type EditorTab = TabBase & {
   ...
   *   避免 tab 打开后连接断开/重连导致 rustSessionId 失效。
   */
...
export function useTabs(initial?: Partial<TerminalTab>) {
  ...
  const bootActiveIdRef = useRef<number | null>(null);
  useEffect(() => {
    if (!booted) return;
    if (bootActiveIdRef.current === null) {
      bootActiveIdRef.current = activeIdRef.current;
    }
    if (activeId === 1 || activeId === bootActiveIdRef.current) return;
    setTabs((curr) => {
  ...
  const warmUpTab = useCallback((tabId: number) => {
    setTabs((curr) =>
      curr.map((t) => (t.id === tabId ? { ...t, cold: false } : t)),
    );
  }, []);
  ...
  useEffect(() => {
    ...
    (
      window as unknown as { __tdsfNewBlockTab?: (cwd?: string) => number }
    ).__tdsfNewBlockTab = newBlockTab;
  }, [newBlockTab]);
