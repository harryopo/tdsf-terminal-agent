...
import type { TdsfLspClient } from "./client";
...
async function createSession(
  ...
  const [{ TauriLspTransport }, { TdsfLspClient }] = await Promise.all([
    import("./transport"),
    import("./client"),
  ]);

  if (TdsfLspClient.hostPid === null) {
    TdsfLspClient.hostPid = await invoke<number>("lsp_host_pid").catch(
      () => null,
    );
  }
  ...
  const client = new TdsfLspClient({
    transport,
    rootUri,
