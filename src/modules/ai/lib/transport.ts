...
export function formatMemoryHintBlock(
  ...
  // #146：旧标题只管**相关性**，没管**时态**，于是记忆里的现在时断言被模型当成本轮事实。
  // 真机取证的形状：某轮处于 OBSERVE、执行类工具被 schema 裁掉，助手说了句
  // 「当前这一轮我这边可用的工具里没有带 shell 映射的执行类工具」——这句话被摘要
  // 沉淀成会话记忆后**每轮回注**，后面带着真终端上下文的回合里模型仍读到
  // 「ssh_command 已从工具集移除、调用会返回 Unknown tool」，就照着历史相信自己没工具
  // （#116「应用对自己的 agent 撒谎」同族，撒谎源头是自己的记忆库）。
  // 这句话必须排在条目**前面**：模型是按顺序读的，先看到断言再看到免责等于没免责。
  const tenseNotice =
    "注意：以下是过去某一次会话的记录，不描述本轮状态。当前的模式、可用工具、" +
    "已连接的服务器与目录一律以本轮的工具清单与 <env>/<live_context> 为准；" +
    "与记忆冲突时以本轮为准，不要因为记忆里写着「当时没有某个工具」就拒绝执行。";
  return {
    block: [tag, header, tenseNotice, ...lines, `</${tag.slice(1, -1)}>`].join(
      "\n",
    ),
