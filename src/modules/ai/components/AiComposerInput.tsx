...
export function AiComposerInput() {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: c.value is the trigger that fires autoresize when the textarea content changes.
  useEffect(() => {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: triggers (snippetTriggerOpen/fileTriggerOpen/fileQuery) are observable values that should reset activeIndex when they change.
  useEffect(() => {
    setActiveIndex(0);
  ...
  return (
    <>
      ...
      {voiceRow.mounted && (
        <div data-state={voiceRow.state} className="tdsf-reveal">
