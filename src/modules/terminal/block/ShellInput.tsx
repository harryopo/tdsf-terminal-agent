...
import { RiskConfirmDialog } from "@/lib/risk-engine/ConfirmDialog";
import { evaluate, shouldBlock } from "@/lib/risk-engine/index";
...
export default function ShellInput({
  ...
  useEffect(() => {
    ...
    const handle = createShellEditor({
      ...
      historyList,
      onSubmit: (text) => {
        // TDSF RiskEngine 拦截：先评估风险等级
        const assessment = evaluate(text);
        if (shouldBlock(assessment)) {
          // L3+ 弹窗确认；L4 deny 弹窗的"已拒绝"按钮也会被 disabled
