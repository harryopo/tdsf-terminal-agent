...
export const FileExplorer = memo(
  forwardRef<FileExplorerHandle, Props>(function FileExplorer(
    ...
    const { lookup: lookupGitStatus } = useGitStatus(
      ...
      gitDecorations && !isRemote ? gitStatus : null,
      gitDecorations,
    );
    ...
    // biome-ignore lint/correctness/useExhaustiveDependencies: tree 对象每次渲染引用都会变化，但 buildRows 只读取其中稳定字段，因此显式列出这些字段。
    ...
    const dnd = useExplorerDnd({
      rootPath: rootPath ?? "",
      isDir: isDirAt,
      onMove: isRemote ? undefined : tree.movePath,
      pathDropTarget: isRemote ? undefined : pathDropTarget,
      disabled: isRemote,
    });
    ...
    return (
      // biome-ignore lint/a11y/noStaticElementInteractions: file explorer is a focusable tree container; keydown handles arrow navigation.
      <div
        ref={containerRef}
        ...
        <div className="flex h-8 shrink-0 items-center gap-1 border-b border-border/60 px-2">
          ...
          >
            <img
              ...
              className="mx-1.5"
            />
            {isRemote && rootPath ? rootPath : basename(rootPath ?? "")}
          </span>
          ...
          <Button
            ...
            size="icon"
            className="size-6 text-muted-foreground hover:text-foreground"
            onClick={() => rootPath && tree.beginCreate(rootPath, "file")}
            title="New file"
          >
          ...
          <Button
            ...
            size="icon"
            className="size-6 text-muted-foreground hover:text-foreground"
            onClick={() => rootPath && tree.beginCreate(rootPath, "dir")}
            title="New folder"
          >
          ...
          <Button
            ...
            size="icon"
            className="size-6 text-muted-foreground hover:text-foreground"
            onClick={() => rootPath && tree.refresh(rootPath)}
            title="Refresh"
          >
              ...
              {menuTarget ? (
                ...
                <>
                  ...
                  <ContextMenuItem
                    className={COMPACT_ITEM}
                    onSelect={() =>
                      rootPath && tree.beginCreate(rootPath, "file")
                    }
                  >
                    New File
                  </ContextMenuItem>
                  <ContextMenuItem
                    className={COMPACT_ITEM}
                    onSelect={() =>
                      rootPath && tree.beginCreate(rootPath, "dir")
                    }
                  >
                    New Folder
                  </ContextMenuItem>
                  ...
                  <ContextMenuItem
                    className={COMPACT_ITEM}
                    onSelect={() => rootPath && tree.refresh(rootPath)}
                  >
                    Refresh
                  </ContextMenuItem>
                </>
