...
export function finishExplorerDrag(
  ...
  onMove: ((from: string, toDir: string) => void) | undefined,
): void {
  const handledByPathTarget =
    commit && (pathDropTarget?.dropPath(source, clientX, clientY) ?? false);
  if (commit && !handledByPathTarget && moveTarget && onMove) {
...
export function useExplorerDnd({
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: placeGhost is a non-memoized helper that closes over refs only; adding it would change the ref callback identity every render.
  const ghostRef = useCallback((el: HTMLDivElement | null) => {
    ghostElRef.current = el;
    if (el) placeGhost(lastPosRef.current.x, lastPosRef.current.y);
  }, []);

  // biome-ignore lint/correctness/useExhaustiveDependencies: placeGhost is a non-memoized helper that closes over refs only; optsRef carries live values. Adding them would change onPointerDown identity every render.
