...
export function AgentLauncherPanel({ onBack, onLaunch }: Props) {
  ...
  const save = (next: AgentLaunchCommands) => {
    ...
    void setAgentLaunchCommands(next).catch((error) => {
      ...
      console.error("[tdsf] failed to save agent launch commands:", error);
    });
  };
      ...
      <div className="mt-3">
        <div className="mb-1.5 flex items-center">
          ...
          <button
            ...
            title={`重置为 ${launcher.defaultCommand}`}
          >
        ...
        >
          {validation.ok ? "支持别名和命令行参数。" : validation.error}
        </div>
      </div>
