...
export function useSourceControl(
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: workspaceKey is the trigger that resets all source-control state when the active workspace changes.
  useEffect(() => {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: workspaceKey is the trigger that re-runs refresh when the active workspace changes; contextPath captures the path.
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: workspaceKey is the trigger that re-runs the auto-refresh setup when the active workspace changes.
  useEffect(() => {
    if (!enabled) {
      requestIdRef.current++;
      setState({
