...
function distroCommand(key: DistroKey, version: string): string {
  switch (key) {
    case "arch":
      return "yay -S tdsf-bin";
    case "debian":
      return `sudo apt install ./TDSF_${version}_amd64.deb`;
    case "fedora":
      return `sudo dnf install ./TDSF-${version}-1.x86_64.rpm`;
  }
}
          ...
          <DialogTitle>
            {ready
              ...
              : downloading
                ? "Downloading update…"
                : manual
                  ? `TDSF v${manual.version} is available`
                  : `TDSF v${update?.version} is available`}
          </DialogTitle>
          <DialogDescription>
            {ready
              ? "Restart TDSF to finish installing."
              : downloading
                ? progress !== null
