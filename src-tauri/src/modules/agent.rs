...
const PI_EXTENSION_FILE: &str = "tdsf-notifications.ts";
const PI_EXTENSION_MARKER: &str = "tdsf-pi-notifications-v1";
const PI_STATUS_NEEDLES: [&str; 6] = [
    PI_EXTENSION_MARKER,
    "agent_start",
    "agent_settled",
    "notify;TDSF;pi;${event}",
    "emit(\"working\")",
    "emit(\"finished\")",
];
const PI_EXTENSION: &str = r#"// tdsf-pi-notifications-v1
import type { ExtensionAPI } from "@earendil-works/pi-coding-agent";

export default function (pi: ExtensionAPI) {
  const emit = (event: "working" | "finished") => {
    if (process.env.TDSF_TERMINAL) {
      process.stdout.write(`\u001b]777;notify;TDSF;pi;${event}\u0007`);
    }
  };
...
const OWNED_MARKERS: [&str; 3] = ["notify;TDSF;", "tdsf;notify", "__tdsf_notify"];
...
fn hook_command(spec: &AgentSpec, event: &str) -> String {
    match spec.delivery {
        Delivery::TerminalSequence => format!(
            r#"[ -n "$TDSF_TERMINAL" ] && printf '{{"terminalSequence":"\\u001b]777;notify;TDSF;{event}\\u0007"}}' || true"#
        ),
...
fn osc_command(agent: &str, event: &str) -> String {
    format!(
        r#"[ -n "$TDSF_TERMINAL" ] && printf '\033]777;notify;TDSF;{agent};{event}\007' > /dev/tty; printf '{{}}'"#
    )
}
...
fn osc_command(agent: &str, event: &str) -> String {
    let exe = std::env::current_exe()
        ...
        .unwrap_or_else(|_| "tdsf-terminal-agent.exe".to_string());
    format!(r#""{exe}" __tdsf_notify {agent} {event}"#)
}
...
fn status_needle(spec: &AgentSpec, event: &str) -> String {
    match spec.delivery {
        Delivery::TerminalSequence => format!("notify;TDSF;{event}"),
        Delivery::Osc => {
            #[cfg(unix)]
            {
                format!("notify;TDSF;{};{event}", spec.agent)
            }
            #[cfg(windows)]
            {
                format!("__tdsf_notify {} {event}", spec.agent)
            }
        }
    }
}
...
fn pi_extension_contents(
    ...
    if existing.is_some_and(|s| !s.trim().is_empty() && !s.contains(PI_EXTENSION_MARKER)) {
        return Err(format!(
            "{} is not managed by TDSF; refusing to overwrite",
            path.display()
        ));
    }
    Ok(PI_EXTENSION)
...
fn write_atomic(path: &std::path::Path, contents: &str) -> Result<(), String> {
    let tmp = path.with_extension("tdsf-tmp");
...
fn conout_marker(agent: &str, event: &str) -> String {
    format!("\x1b]777;notify;TDSF;{agent};{event}\x07")
}

// Windows has no /dev/tty: the hook calls `tdsf-terminal-agent.exe __tdsf_notify ...` and we
...
pub fn emit_conout_marker(agent: &str, event: &str) {
    ...
    if std::env::var_os("TDSF_TERMINAL").is_none() {
        return;
    }
    unsafe {
...
mod tests {
    ...
    fn claude_adds_all_event_hooks_to_empty_config() {
        ...
        assert!(command(&out, "Notification", 0).contains("notify;TDSF;attention"));
        assert!(command(&out, "Stop", 0).contains("notify;TDSF;finished"));
        assert!(command(&out, "UserPromptSubmit", 0).contains("notify;TDSF;working"));
    ...
    fn conout_marker_matches_detector_format() {
        ...
        assert_eq!(
            ...
            "\u{1b}]777;notify;TDSF;gemini;attention\u{7}"
        );
    }
    ...
    fn codex_emits_four_field_dev_tty_marker() {
        ...
        assert!(stop.contains("notify;TDSF;codex;finished"));
    ...
    fn gemini_uses_matcher_and_named_marker() {
        ...
        assert!(command(&out, "AfterAgent", 0).contains("notify;TDSF;gemini;finished"));
        assert!(command(&out, "Notification", 0).contains("notify;TDSF;gemini;attention"));
    }

    #[test]
    fn pi_extension_emits_named_working_and_finished_markers() {
        let path = std::path::Path::new("/x/tdsf-notifications.ts");
        ...
        assert!(extension.contains("process.env.TDSF_TERMINAL"));
        assert!(extension.contains("process.stdout.write"));
    }

    #[test]
    fn pi_extension_only_replaces_its_own_file() {
        let path = std::path::Path::new("/x/tdsf-notifications.ts");
    ...
    fn pi_extension_install_is_atomic_idempotent_and_preserves_foreign_files() {
        let dir = std::env::temp_dir().join(format!("tdsf-pi-extension-{}", std::process::id()));
    ...
    fn pi_extension_install_preserves_symlink() {
        ...
        let dir =
            std::env::temp_dir().join(format!("tdsf-pi-extension-symlink-{}", std::process::id()));
        ...
        let legacy = json!({
            "hooks": {
                "Notification": [
                    { "hooks": [ {
                        "type": "command",
                        "command": "[ -n \"$TDSF_TERMINAL\" ] && printf '\\033]777;tdsf;notify\\033\\\\' > /dev/tty || true"
                    } ] }
                ]
            }
        });
