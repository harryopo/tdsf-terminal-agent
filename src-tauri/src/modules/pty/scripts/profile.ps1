# tdsf-shell-integration (PowerShell)
...
if ($global:__TDSF_HOOKS_LOADED) { return }
$global:__TDSF_HOOKS_LOADED = $true
...
if (Test-Path Function:prompt) {
    Copy-Item Function:prompt Function:__tdsf_user_prompt -Force -ErrorAction SilentlyContinue
}

function global:__tdsf_urlencode {
    param([string]$s)
...
function global:__tdsf_install_readline {
    if ($global:__tdsf_readline_done) { return }
    if (-not (Test-Path Function:PSConsoleHostReadLine)) { return }
    $global:__tdsf_readline_done = $true
    ...
    Copy-Item Function:PSConsoleHostReadLine Function:global:__tdsf_user_readline -Force
    function global:PSConsoleHostReadLine {
        try {
            $line = __tdsf_user_readline
        } catch {
            # Self-heal: restore the original reader so a broken wrapper can
            # never lock the shell out of input.
            Copy-Item Function:__tdsf_user_readline Function:global:PSConsoleHostReadLine -Force
            return ''
        }
        try {
            if ($line -is [string] -and $line.Trim().Length -gt 0) {
                $global:__tdsf_block_seen = $true
                $esc = [char]27
...
function global:prompt {
    ...
    if ($env:TDSF_BLOCKS) {
        ...
        $gap = if ($global:__tdsf_block_seen) { "`n`n" } else { "`n" }
        return "$oscD$oscA$osc7$gap$oscB"
    }

    $original = if (Test-Path Function:__tdsf_user_prompt) {
        try { & __tdsf_user_prompt } catch { "PS $((Get-Location).Path)> " }
    } else {
