...
// OSC 777 marker our agent hooks emit. Legacy 3-field `notify;TDSF;<event>`
// (Claude) or 4-field `notify;TDSF;<agent>;<event>` (Codex/Gemini/Pi).
const TDSF_MARKER: &[u8] = b"notify;TDSF;";
...
impl AgentDetector {
    ...
    fn handle_osc777<F: FnMut(Transition)>(&mut self, pt: &[u8], emit: &mut F) {
        if let Some(tail) = pt.strip_prefix(TDSF_MARKER) {
...
mod tests {
    ...
    #[test]
    fn tdsf_marker_drives_status() {
        let mut d = AgentDetector::new();
        run(&mut d, &osc("133;C;claude"));
        assert_eq!(run(&mut d, &osc("777;notify;TDSF;attention")), vec![Transition::Attention]);
        assert_eq!(run(&mut d, &osc("777;notify;TDSF;working")), vec![Transition::Working]);
        assert!(run(&mut d, &osc("777;notify;TDSF;working")).is_empty());
        assert_eq!(run(&mut d, &osc("777;notify;TDSF;finished")), vec![Transition::Finished]);
    }

    #[test]
    fn tdsf_marker_auto_arms_without_preexec() {
        let mut d = AgentDetector::new();
        assert_eq!(
            run(&mut d, &osc("777;notify;TDSF;attention")),
    ...
    fn four_field_marker_self_arms_named_agent() {
        ...
        let mut d = AgentDetector::new();
        assert_eq!(run(&mut d, &osc("777;notify;TDSF;codex;working")), vec![started("codex")]);
        let mut g = AgentDetector::new();
        assert_eq!(
            run(&mut g, &osc("777;notify;TDSF;gemini;finished")),
    ...
    fn pi_marker_self_arms_and_drives_status() {
        let mut d = AgentDetector::new();
        assert_eq!(
            run(&mut d, &osc("777;notify;TDSF;pi;working")),
            vec![started("pi")]
        );
        assert_eq!(
            run(&mut d, &osc("777;notify;TDSF;pi;finished")),
    ...
    fn four_field_marker_ignores_unknown_agent() {
        let mut d = AgentDetector::new();
        assert!(run(&mut d, &osc("777;notify;TDSF;evil;attention")).is_empty());
        // A known agent in the same chunk still works.
        assert_eq!(
            run(&mut d, &osc("777;notify;TDSF;codex;attention")),
    ...
    fn four_field_marker_drives_status_after_preexec() {
        ...
        assert_eq!(run(&mut d, &osc("777;notify;TDSF;gemini;attention")), vec![Transition::Attention]);
        assert_eq!(run(&mut d, &osc("777;notify;TDSF;gemini;working")), vec![Transition::Working]);
        assert_eq!(run(&mut d, &osc("777;notify;TDSF;gemini;finished")), vec![Transition::Finished]);
    }
