...
_DEFAULT_SYSTEM_PROMPT = (
    ...
    # 2026-09-25 (#140): 他明说「请用表格汇总这台服务器的运行状况：检查项/当前值/
    # 判定/依据命令 四列」，模型给的是 ```text 代码块（近 20 次汇报里只有 1 次自己
    # 出了表格）⇒ 模型不是不会，是没人告诉它默认这样汇报。
    "- 格式约束：不用 emoji（用户要求时除外）；用纯文本或 markdown 结构化表达；"
    "目录树/架构图/流程图放 ``` 围栏代码块保持等宽，勿写进普通段落；"
    "多项检查（巡检/对比/状态汇总）用 markdown 表格：检查项/当前值/判定/依据命令。\n"
    "- 结论要给可执行建议，别停在描述现象。\n"
...
class StrandsAgentAdapter:
    ...
    def _build_prompt(self, input: str, state: dict[str, Any]) -> str:
        ...
        if live.get("sshSessionId"):
            ...
            # TDSF #128 (2026-09-24): **连接活着 ≠ 界面上有一块能写的终端**。用户在偏好里选的是
            # "可见终端执行"，而停在欢迎页时前端一块终端都没挂载（实测 `.xterm` 数 0、
            # terminalRefs 为空），命令会被 #118 那道闸原样拒回来
            # （2026-09-23 真机：三条只读命令 1–6 毫秒全被拒后才汇报失败）。
            # 只在明确收到 "none" 时提示——形状认不出（旧调用方缺字段）就不吭声，
            # 免得把"能用的场景"说成不能用。
            # TDSF #118 后半 (2026-09-25 用户拍板)：这句话必须跟着通道策略一起改。
            # 那道闸现在对**只读/低风险**回 reroute（后台真执行），只有**写操作**才拒；
            # 再写"命令不会被执行"就是拿假事实劝退模型 —— 它会连只读都不肯试。
            if live.get("terminalSession") == "none":
                lines.append(
                    "可见终端: 无——界面上此刻没有任何打开的终端可以写（用户停在欢迎页，"
                    "或工作区面板被换成了别的视图）。连接本身是活的，执行分两半："
                    "只读/低风险命令会自动改走后台通道执行（结果里会写明换了通道，"
                    "界面上看不见这一次），照常调用即可；"
                    "写操作则不执行，同一件写操作不要连续重试——"
                    "请引导用户从顶栏打开该工作区并切到它的终端标签页，再让你执行。"
                )
