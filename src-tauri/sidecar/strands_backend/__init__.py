...
集成方式（生产启动仅支持 Strands；激活失败必须 fail-closed）：
...
def configure_strands(
    ...
    与 RPC 配置门面共享同一份 LLMConfig（环境变量 / .tdsf-data/llm_config.json）。
    ...
    """
    # P0-C5: strands_model=None 时自动注入（与 RPC 配置门面共享同一份 LLMConfig）
