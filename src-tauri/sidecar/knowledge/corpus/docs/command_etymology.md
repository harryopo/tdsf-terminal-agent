...
> 记录所有已学Linux命令的英文全称、词源和中文含义。
> ⭐ 标注的是四级（CET-4）词汇，帮助关联英语学习。
> 所有命令注释必须包含英文全称，帮助学生理解命令含义。
...
| getfacl | get file access control list | 获取ACL | |
| setfacl | set file access control list | 设置ACL | |
...
| rpm | Red Hat Package Manager | RPM包管理 | |
| firewall-cmd | firewall command | 防火墙命令 | |
| export | export | 导出环境变量 | ⭐ |
| source | source | 在当前shell执行脚本 | ⭐ |
| alias | alias | 别名 | ⭐ |
...
| ssh-keygen | ssh key generate | SSH密钥生成 | |
| ssh-copy-id | ssh copy identity | SSH公钥复制 | |
| rsync | remote synchronize | 远程同步 | |
| wget | web get | 网络下载 | |
| curl | client URL | URL请求工具 | |
| ss | socket statistics | 套接字统计 | |
| netstat | network statistics | 网络统计 | |
| firewall-cmd | firewall command | 防火墙命令行工具 | |
...
| fg | foreground | 前台 | |
| bg | background | 后台 | ⭐ |
| crontab | cron table | cron任务表 | |
| crond | cron daemon | cron守护进程 | |
...
| tuned-adm | tuned administration | tuned管理工具 | |
| nice | nice | 进程优先级调整值 | ⭐ |
| renice | renice | 重新调整进程优先级 | |
| chrt | change real-time | 修改实时调度策略 | |
| at | at | 一次性定时任务 | ⭐ |
| atq | at queue | 查看at任务队列 | ⭐ |
| atrm | at remove | 删除at任务 | ⭐ |
| timedatectl | time date control | 时间日期控制 | |
...
| xfs_growfs | XFS grow file system | 扩展XFS文件系统 | |
| resize2fs | resize ext2/3/4 file system | 调整ext文件系统大小 | |
...
| vdostats | VDO statistics | VDO统计信息 | |
| exportfs | export file systems | 导出文件系统 | ⭐ |
| showmount | show mount | 显示NFS挂载信息 | |
| rpcinfo | RPC information | RPC信息查询 | |
| rpcbind | RPC bind | RPC绑定服务 | |
...
| semanage | SELinux manage | SELinux策略管理 | ⭐ |
| restorecon | restore context | 恢复安全上下文 | |
| setsebool | set SELinux boolean | 设置SELinux布尔值 | |
| getsebool | get SELinux boolean | 获取SELinux布尔值 | |
| sealert | SELinux alert | SELinux告警分析 | |
...
| gzip | GNU zip | GNU压缩 | |
| gunzip | GNU unzip | GNU解压 | |
| bzip2 | Burrows-Wheeler compress 2 | bzip2压缩 | |
| xz | xz | xz压缩 | |
| zip | zip | zip压缩 | |
| unzip | unzip | zip解压 | |
...
| 命令 | 英文全称 | 中文含义 | 四级 |
|------|---------|---------|------|
| journalctl | journal control | 系统日志控制 | |
| logger | logger | 写入日志 | |
| chronyc | Chrony client | Chrony时间同步客户端 | |
| last | last | 显示最近登录 | ⭐ |
...
| 文件路径 | 英文含义 | 中文作用 | 四级 |
|---------|---------|---------|------|
| /etc/passwd | password file | 用户信息文件（7字段） | ⭐ |
| /etc/shadow | shadow file | 密码哈希文件（仅root可读） | |
| /etc/group | group file | 组信息文件 | ⭐ |
| /etc/fstab | file system table | 文件系统挂载表（6字段） | ⭐ |
| /etc/hosts | hosts file | 主机名与IP映射 | ⭐ |
...
| /etc/exports | exports file | NFS共享配置 | ⭐ |
...
| /etc/shells | shells file | 有效登录shell列表 | ⭐ |
...
| /proc/cpuinfo | CPU information | CPU信息 | ⭐ |
| /proc/meminfo | memory information | 内存信息 | ⭐ |
| /proc/version | version | 内核版本 | ⭐ |
| /boot/grub2/grub.cfg | GRUB configuration | GRUB主配置（勿手动编辑） | |
...
| ALU (Arithmetic Logic Unit) | 运算器 | | 算术逻辑运算 |
| CU (Control Unit) | 控制器 | | 指令控制 |
| cache | 缓存 | ⭐ | 高速缓冲存储器 |
| bus | 总线 | ⭐ | 数据传输通道 |
| interrupt | 中断 | | 外部/内部中断机制 |
| operating system (OS) | 操作系统 | ⭐ | 系统软件 |
| kernel | 内核 | ⭐ | OS核心 |
...
| ready / running / blocked | 就绪/运行/阻塞 | | 进程三状态 |
| virtual memory | 虚拟内存 | ⭐ | 主存-辅存两级存储 |
| paging | 分页 | | 固定大小页面 |
| segmentation | 分段 | | 逻辑段 |
| swap | 交换 | | 内存与磁盘交换 |
| locality of reference | 局部性原理 | | 时间局部性+空间局部性 |
| multi-user | 多用户 | ⭐ | Linux特点 |
| multi-tasking | 多任务 | ⭐ | Linux特点 |
...
| HTTP (HyperText Transfer Protocol) | 超文本传输协议 | ⭐ | Web协议 |
| HTTPS (HTTP Secure) | 安全超文本传输协议 | | 加密Web协议 |
| FTP (File Transfer Protocol) | 文件传输协议 | ⭐ | 文件传输 |
| SSH (Secure Shell) | 安全壳协议 | ⭐ | 安全远程登录 |
| NFS (Network File System) | 网络文件系统 | | 网络共享文件 |
| SMB/CIFS | Server Message Block | | Windows文件共享 |
| subnet mask | 子网掩码 | | 网络划分 |
| gateway | 网关 | ⭐ | 网络出口 |
| port | 端口 | ⭐ | 服务标识 |
| firewall | 防火墙 | ⭐ | 网络安全设备 |
| zone | 区域 | | 防火墙区域管理 |
| masquerade | 地址伪装 | | NAT功能 |
| port forwarding | 端口转发 | | 端口映射 |
| rich rule | 富规则 | | 高级防火墙规则 |
| MAC address (Media Access Control) | 物理地址 | | 网卡硬件地址 |
| ARP (Address Resolution Protocol) | 地址解析协议 | | IP→MAC映射 |
| loopback | 回环接口 | | 127.0.0.1 |
| network interface | 网络接口 | | 网卡设备 |
| Ethernet | 以太网 | ⭐ | 局域网技术 |
| RPC (Remote Procedure Call) | 远程过程调用 | | NFS依赖的机制 |
...
| hard link | 硬链接 | | 同一inode |
| symbolic link / soft link | 符号链接/软链接 | | 独立inode |
| GPT (GUID Partition Table) | GUID分区表 | | 新分区表，无2TB限制 |
| MBR (Master Boot Record) | 主引导记录 | | 旧分区表，≤2TB |
| ext4 (Fourth Extended FS) | 第四代扩展文件系统 | | Linux常用 |
| xfs (X Filesystem) | X文件系统 | | 高性能文件系统 |
| swap space | 交换空间 | | 内存扩展 |
| disk quota | 磁盘配额 | | 用户空间限制 |
| PV (Physical Volume) | 物理卷 | | LVM底层 |
| VG (Volume Group) | 卷组 | | LVM中层 |
| LV (Logical Volume) | 逻辑卷 | | LVM上层 |
| extend / resize | 扩展/调整大小 | ⭐ | LVM扩容操作 |
| deduplication | 去重 | | VDO功能 |
| compression | 压缩 | ⭐ | VDO功能 |
| zero block clearing | 零区块清除 | | VDO三阶段之一 |
...
| nice范围 | -20（最高优先级）到 19（最低优先级），默认0 |

---

## 🔄 自动更新规则

1. 每次处理新项目后，将新命令的英文全称追加到对应分类
2. 如果发现新的四级词汇标注遗漏，补充标注
3. 如果新命令的选项有新的英文来源，追加到常用选项表
4. 新增的技术术语追加到对应分类表
