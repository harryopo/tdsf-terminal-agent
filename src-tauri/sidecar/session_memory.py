...
# 时态红线（#146）：这份摘要会被回注到**以后**的对话里。真机取证：某轮处于 OBSERVE、
# 执行类工具被 schema 裁掉，助手说了句「当前这一轮我这边可用的工具里没有带 shell 映射的
# 执行类工具」——这句**现在时**的自我说明被沉淀成会话记忆后每轮回注，后面带着真终端
# 上下文的回合里模型仍读到「ssh_command 已从工具集移除」，就照着历史相信自己没工具。
# 读侧另有兜底（transport.ts 的时态声明），这里管住"别再往里写新的"。
_SUMMARY_PROMPT = (
    "你是 Linux 运维教学助手的会话记录员。请把以下师生对话压缩为一份结构化摘要，"
    "供未来排查类似问题时检索参考。摘要必须包含以下小节（没有的小节写「无」）：\n"
    "## 问题现象\n## 根因分析\n## 解决命令（逐条列出实际用到的命令）\n## 教学要点\n\n"
    "要求：忠实于对话内容，不编造；命令保留原样；总长 500 字以内。\n"
    "时态要求：这份摘要将来会被回注到别的对话里，所以只写可复用的做法与结论，"
    "不要写任何一轮的即时状态——当前模式（AUTO/CONFIRM/OBSERVE）、本轮有哪些工具、"
    "某工具是否被禁用或返回 Unknown tool、此刻连的是哪台主机或在哪个目录，一律不写；"
    "对话里出现「当前/本轮……」这类助手自我说明时跳过它，只保留它背后的经验。\n\n"
    "---\n\n{transcript}"
)
