...
**问题与决策**：原先的 `SshCommandOutputPanel` 订阅原生 SSH 流后在对话底部额外渲染一张“SSH Output”面板。它与触发命令的 `SSH` 工具卡割裂、重复展示同一事实，用户无法在单一折叠卡中追踪一次命令。

**修复**：删除对话层独立面板与其测试；流式事件保留在 sidecar adapter 中用于长任务活动感知。在 `ssh_command` 工具仍处于 `input-streaming` 时，工具卡自动展开，并按当前会话及命令原文接收 stdout/stderr/status 块，在卡内实时追加原生回显；最终结构化结果继续由同一工具卡既有输出组件展示。输出上限维持 64 KiB，避免异常命令拖慢界面。

**验证与边界**：新增工具卡定向回归，覆盖逐块追加、完成状态和跨会话/跨命令隔离，**2/2 通过**；`pnpm typecheck`、`pnpm lint`、`git diff --check` 通过。没有把浏览器 mock 当作桌面验收：用户应在原生桌面运行动态 `dnf` 或长输出 SSH 命令，确认无独立“SSH Output”面板，实时内容只在对应 SSH 工具卡中更新且可折叠。

### 37.134 教学单步命令链与 TeachCard 渲染契约收口（2026-09-09 ✅，待用户原生复测）

**问题与根因**：用户的真实教学日志显示，模型在教学 observe 中调用了普通 `suggest_command`；成功和未匹配结果都以“命令建议 / error”的原始 JSON 工具记录出现，而不是教学命令卡。根因不是学生操作，而是教学 schema 错保留了该工具，通用 observe prompt 又要求使用它。卡片标记还依赖模型自行输出，漏标记时会退回普通 Markdown；标记被单独流式传输时，前端仍渲染空的 `MessageResponse`，于是两条回复间留下空白。模型也没有受运行时限制，一次可能请求多条命令。

**修复**：教学 observe 改用独立工具过滤，只保留 shell 映射工具和知识/提问等无终端副作用辅助工具，排除 `suggest_command`、`todo_write`、`get_terminal_output`。教学专用提示词覆盖 observe 的命令建议与任务规划文本：每轮先产生且只产生一张可见终端命令卡，附一两句观察提示即停止；只有学生点击、终端完成、再点击“基于结果继续讲解”提交的 `<teaching-command-result>` 可以驱动下一步。工具包装器共享单轮门闩，第二次调用返回内部等待状态而不执行也不泄露 JSON；无法映射的工具同样 fail-closed。后端在首个教学输出分片强制注入唯一 `tdsf:teach` 标记，前端清除仅作传输用途的标记空片段，卡片恢复稳定渲染且不再制造留白。卡片新增“预期回显”，只描述命令类别与可能的成功/失败信号，不伪造当前主机的值。

**验证与边界**：完整 sidecar pytest 首跑为 **2231 passed / 1 failed**；唯一失败是旧测试仍断言“模型自行写标记”的已废弃提示词，更新断言后受影响的 **276 passed**。教学 Python 定向回归 **24 passed**，前端 TeachCard/命令卡定向 **11 passed**，`pnpm typecheck`、`pnpm lint`、`cargo check`、`git diff --check` 通过。未用浏览器 mock 冒充桌面验收，也未改变普通观察、确认或自动模式：教学命令只能由学生点击后写入当前可见终端。用户原生复测应确认首轮只有一张紫色步骤卡、没有“命令建议 error”，命令在当前终端显示并完成后，下一张卡才会出现。
...
**修复与验证**：两个按钮直接绑定既有异步处理函数，保留处理期间禁用状态。新增组件回归，先证明批准/拒绝回调均为 0 次，再修复为 **2/2 passed**；全量前端门禁由 Luna 执行。未改动 TOFU 安全策略、known_hosts 或 SSH 后端。
...
**最小实现**：新增 `os-family.ts`，仅标记明确互斥命令；SSH 连接后并行预取远端命令全集和既有 sidecar probe。命令过滤、尾部参数触发、carapace/tldr/Fig 参数加载统一检查同一 session 缓存。`history`、未标注命令、`unknown` 永远保留；探测失败/老 sidecar/异常 schema 退化为 `unknown`。没有新增 PTY 注入、没有从 `PRETTY_NAME` 猜测，也没有为了展示而伪造状态。

**验证与边界**：先跑出 5 个红测，再实现后定向 Vitest **94 passed**；typecheck、lint、diff check、全量 Vitest **131 files / 1349 tests**、Web build 均通过。M1-2b 的状态栏仍未实现：当前 probe cache 不是 reactive UI store，提前显示“已识别”会造成陈旧状态；真实 Tauri/SSH 回归也因自动化不具备原生应用 surface 而保持待验收。

**复盘**：稳定性来自单一事实源和可解释降级，不是扩大命令映射表。只要 source 不确定，宁可完整展示而非制造“聪明”的误过滤；真正的状态 UI 必须先拥有 session 生命周期和订阅证据。
...
### 37.104 spec Task 9/10 逐行复核：三项不能核销，抓到一条假绿用例（2026-09-02 ⚠️→#45 挂号）

**做法**：派 general-purpose 子代理只读勘察（不允许多改文件），要求每条宣称给「文件:行号 + 判定（已实现有测试 / 已实现无测试 / 不符或未实现）」，并额外回答"这些 T9 测试会不会依赖真实网络=假绿"。我给的输入是 §37.100 里的**宣称**，明确要求它当作待验证内容。

**结论**：**可核销 2 项**——T9.3 并行工具提示词（`adapter.py:194` + `test_watchdog.py:144-155` 断言）、T10.2 证据三段分组（`evidence.ts:73-129` + `AiChat.tsx:371-390` + `evidence.test.ts` 16 例）。**不可核销 3 项**（见 ROADMAP #45）：
1. **T9.1**：`timeout=300/max_retries=2` 只加在 OpenAI 兼容分支，**Anthropic 分支漏了**；32 例 model_adapter 测试对这两个参数零断言（讽刺的是那个 Mock Model 已经在记录 `client_args`，加断言是现成的）。watchdog 实现齐全，但 8 例里只有 3 例属 T9.1，且**全都直接调 `_wait_with_watchdog`**——`invoke()` 那条真链路（worker 起停 / agent_log 落盘 / 降级响应体 / 解除 stalled 标记）一条断言都没有。
2. **T9.2 抓到一条假绿**：`test_watchdog.py:129-138` 函数名叫 `test_invoke_degrades_friendly_on_transport_error`，**实际只断言分类函数返回 True，从未进入 invoke**。名字承诺了它没验证的事——这比"没测试"更危险，因为它会让下一个 AI（和我）以为这条路已经过验证。11 个传输错误特征也只测到 7 个。
3. **T10.1**：后端 `core/confidence.py:457` DSPCR5 分档权重确实未固化（全仓 grep 无 `分档/tier`）；而且 checklist 那句「高档展示依据来源」**实质未满足**——阈值 ≥0.5 时前端什么都不显示，"展示依据"只发生在低档。

**顺带查出的工程隐患**（都记进 #45，不在本轮动）：`adapter.py:1073` 的 `max(1.0, float(raw))` 把测试设的 0.5s 阈值钳成 1.0s，用例是靠 worker 睡 2.0s **侥幸**触发而非真正验证"活动续期"；超时文案硬编码"10 分钟"不随 env 阈值变；`test_watchdog.py:45-46` 的 tearDown 只 pop IDLE，`POLL_SECS=0.1` 泄漏给同进程后续用例；被弃管的 worker 仍持有 `agent_lock` 在后台跑，只靠会话级标记短路；活跃信号取自 `getattr(handler,"_stats",None) or {}`，该结构一变就恒为 0 → 600s 误判超时。
...
### 37.107 UI 六项修改（模式选择器下沉底栏 / 删 Ctrl+I UI / SSH 地址 / 工作区去重叠 / 打字机自动执行 / 置信度按模式）（2026-09-02 ✅）

**缘起**：用户实测提出一批 UI 问题（附 4 张截图）：① 模式选择弹窗放对话区显乱，要下沉到底部状态栏 Strands 旁的"权限 UI"位置；② 删 Ctrl+I 的 UI（用户以为 Ctrl+I 切模式，实为开 AI 小窗，无模式切换快捷键）；③ 默认就用截图那种四档复杂抽屉；④ 连服务器时底部应显示服务器地址而非 "Windows"；⑤ AI 对话框顶部工作区 UI 与历史对话概要（SessionPicker）重叠；⑥ 打字机应"AI 一写命令就自动输出到终端"，而非手动点 Run；⑦ "置信度 低：未检索到可靠来源佐证"每条都弹很鸡肋，只在教学/确认模式分析。用户就打字机行为明确选择"自动打字+自动执行"。

**六项改动（全前端，12 文件）**：
1. **模式选择器下沉底栏**：`StatusBar.tsx` 右侧用 `AgentModeSwitcher`（交互式四档抽屉）取代原只读 `AgentStatusPill`，顺序 `[MockLLMWarning][AgentModeSwitcher][BackendPill(Strands)][AiStatusBarControls]`；从 `AiComposerInput.tsx`（对话输入区）与 `TdsfAgentPanel.tsx`（已弃用面板）移除切换器保持对话区干净。busy/循环进度反馈仍由顶栏 Header 的 AgentStatusPill 承载（未动）。连带清理 `onOpenMini` prop 链（StatusBar Props + App.tsx 的 `openMini` 变量与传参）。
2. **删 Ctrl+I UI**：`AiStatusBarControls.tsx` 删除显示 "Ctrl+I" kbd 的关闭面板按钮（连带 `closePanel` 变量）+ 删除死代码 `AiOpenButton`（含 "Open AI agent"+Ctrl+I 标签，全仓无引用）+ 清理失效 `Kbd` 导入。**Ctrl+I 快捷键本身保留**（仍是开 AI 小窗主入口）。
3. **SSH 显示服务器地址**：`WorkspaceEnvSelector.tsx` label 增 `env.kind==="ssh"` 分支显示 `user@host`；放开 `!IS_WINDOWS` 隐藏限制（非 Windows 仍显示 SSH）；下拉 WSL 段仅 Windows 显示、本地项文案平台化；trigger `max-w-28→44` 容纳地址。
4. **工作区去重叠**：`AiMiniWindow.tsx` 顶部 Header 移除 `WorkspaceChip`（窄窗下与右侧 SessionPicker 重叠），改挂到 Body 底部（messages 与 TodoStrip 之间的轻量 footer，保留点击开工作区总览）。
5. **打字机自动执行**：`chatStore.ts` 的 `autoExecuteInTerminal` 默认 `false→true`；`chat-code.tsx` 的 `CommandCard` 加自动注入 useEffect（渲染后 `injectIntoActivePty(code+"\n")`，`autoFiredRef` 保证每卡只注入一次，NOOP_LIVE 下返回 false 安全无副作用），`RunInTerminalButton` 改受控展示组件（sent/onRun props），与 `tool.tsx` SuggestCommandCard 同源。
6. **置信度按模式**：`AiChat.tsx` 的 `ConfidenceMarker` 加 `agentMode` 订阅，仅 `teach`/`confirm` 两档评分并显示，`observe`/`auto` 直接 `setResult(null)` 不弹标签。

**⚠️ 关键安全发现与解耦决策（打字机项，务必记住）**：`autoExecuteInTerminal` 此前**同时**驱动两条路径——(a) 前端对话命令卡注入（CommandCard/SuggestCommandCard 直接 `injectIntoActivePty`，单次执行，安全）；(b) 经 `chatRuntime.ts` 下发 sidecar → `ssh_command.py` 的 `if ctx.auto_execute_in_terminal: visible=True` → 发 `inject_terminal` 通知 → 前端 `useAiLiveBridge.ts` 注入 PTY（autoExec 时 +\n 执行）**且** `execute_via_ssh` 后台 exec 通道也执行 = **同一服务器命令双重执行**（对 `echo>>file`/`useradd`/服务重启等非幂等命令有害）。这是 visible 模式（2026-08-09 P1#15 上线）自带的潜伏特性，仅在手动开 autoExec 时触发；默认开会把它常态化。**处置（红线9：不在 UI 任务里动 SSH 后端链路）**：在 `chatRuntime.ts` 两处（SSH/local 分支）把下发 sidecar 的 `autoExecuteInTerminal` **解耦为 `false`**——前端标志专用于对话命令卡自动执行，不再自动触发 ssh_command visible。sidecar 侧 visible 保持默认关闭（与历史 autoExec=false 行为一致，**无回归**），AI 仍可显式传 `visible=true`。**遗留（需独立红线9任务）**：ssh_command 工具调用的"可见终端执行"要真正做到无双重执行，须重构为 PTY 执行 + scrollback 回读、跳过 execute_via_ssh（inject_terminal 注入交互式 PTY 存在两难：+\n 双执行 / 不加 \n 残留悬空命令污染用户下次输入）。

**另一处根因修复（SSH 地址项）**：全局 `useWorkspaceEnvStore.env` 在**初次加载 SSH Space 时不会被 `adoptWorkspaceEnv` 同步**——`App.tsx:347` 的 `prevSpaceRef` 有 `prev===null||prev===activeSpaceId` 早退守卫，首帧不 adopt，全局 env 停在 LOCAL_WORKSPACE → 底部仍显示 "Windows"（正是用户截图 "Windows / root" 的根因）。改 `WorkspaceEnvSelector` 标签以**活跃 Space 的 env**（`useSpaces` 持久化真源，`s.spaces.find(...)?.env` 返回稳定引用合红线6）为准、回退全局 env，绕过同步时序 gap。
...
**缘起**：用户"整理目录文件各级分类，安全删不需要的，更新readme/claude核心文件，修改不合理约束，提高开发效率质量"。

**勘察发现**：根目录12个临时log(~14MB,untracked gitignore)；**无根README**(只有docs/README是上游terax遗留引TERAX.md失效)；CLAUDE.md门禁"982"过时(实1327)+版本v2.2(2026-08-09)；docs 237文件顶层33.md多被引用。

**整理（保守策略，用户选）**：①删12根目录log(DeleteFile,untracked安全,清14MB) ②创建根README.md(项目门面:简介/特性/技术栈/快速开始/四档信任/架构/文档索引/开发规范) ③docs/README.md去上游TERAX.md失效引用改中文docs索引指向CLAUDE.md ④CLAUDE.md v2.2→v2.3(门禁982→动态约1327+§5补真实运行调试方法论) ⑤.gitignore加cdp-dir。

**docs归类保守决策**：方案书v1.0/v1.1被DEV-JOURNAL L4/P3-SSH方案/Agent架构说明书精确路径引用(docs/方案书-v1.0.md)，移动会断引用→按保守原则不移动保持顶层；用docs/README.md索引清晰化替代物理归类。
...
### 37.136 可见终端 Agent 蓝色命令回显（2026-09-10 ✅）

**用户诉求**：可见终端中的 Agent 命令需以蓝色明确区分，且必须是实际写入 SSH/PTY 后的原生回显，不能为宣传片伪造画面。

**实现**：在前端终端输出渲染边界新增一次性字节匹配器。命令仍按原样通过 SSH/PTY 写入；只有收到与已注入命令匹配的回显字节时，xterm 才临时插入蓝色 SGR 样式，随即复位。命令输出、提示符、用户输入和远端实际收到的字节均不改动。逐字输入中断或写入失败会清除待匹配标记，避免错误染色。

**验证**：新增 Vitest 覆盖跨 chunk 回显着色和非匹配输出字节不变；定向 2/2 通过，`pnpm typecheck` 通过。待用户在桌面端以“可见终端 Shell”执行一次 Agent 命令，确认命令为蓝、原生输出保持默认色。

### 37.137 宣传片制作交接与下一步收敛（2026-09-10）

**用户决策**：宣传片改交由其他 AI 制作。保留已确认的最终字幕、约 100–112 秒的分镜、真实素材清单与“只交付带 BGM 版”约束；不再在本仓库继续制作视频。

**交接物**：`promo-video/docs/production-spec.md` 是唯一脚本与分镜来源，`promo-video/HANDOFF.md` 明确素材、产品事实、隐私规则和未采集镜头。已准备的截图仅来自真实桌面；本环境的桌面自动化接口未枚举到原生 TDSF 窗口，故不能伪造录屏或 UI 补位。

**工程优先级更正**：近期主线不再是视频，而是用户原生复测后收口可见终端蓝色命令、动态 SSH 输出、提问暂停、教学单步与跨服务器资源树；之后先完成工具调用 ID/lifecycle 的统一，再评估恢复安全的并行工具执行。旧浏览器 mock 不能替代 Tauri/SSH/PTY 桌面证据。

### 37.138 教学模式独立命令卡 Schema 兜底（2026-09-10 ✅）

**问题**：真实教学会话曾只向模型暴露 `ask_user`、SSH 会话枚举与知识库等辅助工具。模型因此正确地拒绝凭空构造命令卡，但教学被降级为纯文字，前端自然没有 `teach_command` 可渲染。

**最小修复**：保留既有“运维工具 shell 映射 → 教学命令卡”路径，并新增只在 `teach + observe` 运行时注册的 `teach_command` 工具。它是 schema 中明确可用的单步教学卡原语：不调用 SSH、不在后端执行，只返回 `status=teach_command` 交给现有 `TeachCommandCard`；学生点击后才会通过当前可见终端打字机执行。它共享每轮一张卡的等待锁，并复用 `command_impact` 拦截 denylist。这样部署类教学命令也不再依赖某个诊断工具恰好存在参数映射。

**验证**：Python 定向测试 **16 passed / 10 skipped**；覆盖教学运行时 schema 同时包含 `teach_command` 与 `ssh_command`、专用卡不执行命令、单步锁和硬黑名单拒绝。另修正无 `strands` 环境下被跳过集成测试仍在导入期引用未定义 `Model` 的测试兜底；不影响生产侧。

**真实验收**：在桌面端切至“教学”，询问“教我部署一个 Web 网页”。首轮应产生一张紫色教学命令卡（而非“没有 shell 映射工具”的文字）；点击后命令仅进入当前可见终端，完成回显后点击“基于结果继续讲解”才出现下一张卡。

### 37.139 命令建议与教学命令卡统一（2026-09-10 ✅）

**问题**：命令建议和教学命令卡虽然都有命令与预测回显，卡面却不一致：教学卡没有 `BASH` 标识和复制入口，建议卡也缺少统一的命令卡表头；模型有时把可执行命令塞进正文或反引号，绕过了终端交互入口。
...
**问题**：教学工具行仍会先展示重复的 JSON Input，再展示命令卡；同时真实 Bash 在 `a; b` 或管道命令下可能只把首段写入终端完成块。教学关联此前坚持全文精确匹配，首张卡即使已提交也会等满 90 秒，进而占住终端并阻止下一张卡运行。
...
### 37.145 打字机回显即时转发（2026-09-10 ✅）

**用户复测**：打字机模式再次表现为“没有逐字输入”。Rust 的 human_type pump 仍按字符写入，但前端 `AgentCommandEcho` 为了等待整条命令匹配，把每个 PTY 回显块缓存到命令结束后才交给 xterm，因此逐字回显被人为隐藏。
...
**用户复测**：教学回答再次退回成整段普通文本，概念、操作示例和易错提示没有卡片层次；Run 注入后的 `[root@server ~]#` 出现蓝白交错，命令颜色与提示符混在一起。

**根因与修复**：前端旧逻辑只有在完整响应带 `<!-- tdsf:teach -->` 且非流式时才切换 TeachCard，旧 sidecar/历史响应只输出加粗或纯文本教学标题时自然落回 Markdown。现在显式教学标记在流式期间即可进入卡片，Teach 模式完成后还兼容“概念与原理 / 路径 / 操作示例 / 易错提示 / 练习”等加粗、编号或纯文本标题；普通模式和知识检索仍不会因标题自动课程化。TeachCard 增加紫色教学头部、板块索引、编号、概念/示例/易错/练习分色层次，保留命令复制与插入终端入口。

提示符混色来自即时回显匹配器：`rpm`、`ss` 等命令的首字节可能先在 `root`/`server` 提示符里命中。人类打字和教学 Run 的回显现在先等待 `# `、`$ `、`% ` 或 `> ` 提示符边界，再开始命令匹配；提示符和普通输出原样透传，OSC/CSI 控制序列保护逻辑继续生效。瞬时整段写入路径保持原有不等待边界的兼容行为。

**验证**：TeachCard/提示符回显定向 Vitest **16 passed**；`pnpm typecheck`、定向 ESLint、Prettier 及 `git diff --check` 通过。用户重启桌面端/sidecar 后，新开教学回合应看到分区卡片；点击 Run 时 `[root@server ~]#` 保持单一终端颜色，仅命令文本显示 Agent 蓝色。
