...
| 43 | **用户实测问题批次一（agent 体验专项，用户钦定"底层→UI 逐个解决"）**：A1 会话按服务器隔离（SessionMeta.scope 创建时绑定 user+host+port，getLive 按 scope 解析，会话列表徽章，老会话兼容）✅；A2 max_tokens 自动续跑 ≤3 轮（方案书 T9 精神）✅；B1 欢迎页环境误报修复（无终端时 cwd/workspace 置 null→引导分支）✅；C1 审批卡命令完整换行显示 ✅；C2 影响预测（docker ps 只读放行+对象去噪）✅；C3 打字机无效+命令输出不回显终端 ⏳ 待真机排查（inject_terminal 链路）；C4 教学卡流式过渡区裸 markdown ⏳ 待真机复现；D1 自测底座（tauri:dev 后台自跑+sidecar 日志+agent-logs）✅ | 🔶 五修完成（3f4c70e），C3/C4 待下一批真机调试 | 2026-09-01；pytest 2053(+9)/vitest 376/tsc/lint 0/build 全绿；**AI 已获授权自行启动软件自测**（用户 2026-09-01 钦定，不再依赖用户桌面实测）；方案书 v4.0 P2 T8-T10 顺延 |
