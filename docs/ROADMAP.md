...
### 2026-09-10 SSH 实时回显归位

- [x] 删除对话层独立 `SSH Output` 面板，不再在工具调用之外重复展示原生输出。
- [x] 流式 stdout/stderr/status 仅在对应 `ssh_command` 工具卡内实时追加；运行时自动展开，结束后仍在同一卡片内查看结构化结果。
- [x] 自动化：工具卡回归 2 项、typecheck、lint、`git diff --check` 通过。
- [ ] 用户原生复测：运行动态 `dnf` 或长输出 SSH 命令，确认对话底部没有独立面板，回显只在当前 SSH 工具卡内更新且可折叠。

### 2026-09-09 教学单步命令链与 TeachCard 渲染收口

- [x] 教学 observe 使用独立工具集：只保留可映射为可见终端命令的工具，以及知识/提问等非终端辅助工具；移除 `suggest_command`、`todo_write`、`get_terminal_output` 的降级入口。
- [x] 每个教学回合最多生成一张命令卡；卡片展示安全的“预期回显”，学生在当前终端执行并提交精确结果后，Agent 才能给下一步。
- [x] 后端强制写入 `tdsf:teach` 标记，前端过滤标记专用空片段，修复模型漏标记导致卡片退回 Markdown 与回复间留白。
- [x] 自动化：完整 sidecar pytest 首跑 2231 项通过，仅 1 条旧提示词断言随契约更新；受影响的 276 项复跑通过。前端卡片定向 11 项、typecheck、lint、cargo check 通过。
- [ ] 用户原生复测：教学模式提出“教我对待一个新系统应该怎么做”，确认首轮仅一张紫色步骤卡且无“命令建议 error”；点击后命令只输入当前可见终端，回显完成并点击“基于结果继续讲解”后才出现下一张卡。

### 2026-09-09 SSH 资源管理器跨服务器一致性

- [x] 审计确认无 `192.168.45.200` 特化；资源树按当前 SSH Space 的 session 与 cwd 取数。
- [x] 修复相同远程 cwd 切换不同会话时不重载目录树、展开状态串会话以及远程源误注册本地 watcher。
- [x] 启用远程资源管理器的系统文件拖放：本地文件通过当前 SSH session 的 SFTP 上传，成功后刷新目标目录。
- [x] 兼容无 `getent` 的最小 Linux shell 探测，恢复 bash/zsh/fish 会话的 OSC cwd 上报。
- [x] 自动化门禁：Vitest 1360、typecheck、lint、Web build、cargo check、Rust 库测试通过。
- [ ] 用户原生复测：连接 `.128` 后 `cd /tmp`、切回 `/root`，确认左侧树随当前会话更新；把本地文件拖到树中，确认文件仅出现在该服务器的目标目录。

### 2026-09-09 SSH 执行链与 Agent 网络排障收口

- [x] 修复 Python→Rust 桥固定 30 秒早退；桥接等待晚于远端命令语义超时。
- [x] Rust SSH exec 逐块推送 stdout/stderr，超时保留部分输出；前端按会话显示原生实时回显并维持长任务活动状态。
- [x] 新增 `ask_user` 结构化提问工具和前端回答卡；用户回答前暂停，取消/拒绝后不再触发 todo/验证自动追加轮。
- [x] 新增 `network-troubleshoot` 内置 Skill；强化网卡、路由、网关邻居、DNS、仓库连通性及虚拟机 NAT/桥接/Host-only 分层定位。
- [x] 自动化门禁：Vitest 1356、typecheck、lint、Web build、cargo check、Rust 库/集成/doc tests 通过；sidecar 全量 **2231 passed / 2 warnings**。
- [ ] 用户原生复测：后台模式执行 35 秒以上与动态 `dnf` 命令，确认 output 边产生边显示且最终状态按退出码变绿/变红。
- [ ] 用户原生复测：让 Agent 在 NAT/桥接等实质方案间选择，确认只出现一张提问卡、回答前不继续、回答后恢复同一任务。
- [ ] 后续：为原生 SSH 流式事件补保存日志/重连取证，继续统一剩余工具的显式 `tool_call_id` 生命周期；不恢复猜测性并行或自动重试。

### 2026-09-09 SSH 首次连接 TOFU 交互修复

- [x] 修复“信任并连接”和“拒绝”按钮只创建处理函数却未执行的问题。
- [x] 新增批准/拒绝点击回归测试，验证两个回调都能准确触发一次。
- [ ] 用户原生复测：新主机确认指纹后点击“信任并连接”，确认弹窗关闭且原连接继续完成。
...
- [x] **教学命令卡 Schema 兜底**（2026-09-10，DEV-JOURNAL §37.138）：教学运行时固定暴露 `teach_command`，部署类课程也能生成一张可见终端教学卡；不在后端执行，单步等待与硬 denylist 不变量保持不变。
- [x] **统一命令建议与教学命令卡**（2026-09-10，DEV-JOURNAL §37.139）：两类卡统一 `BASH / Run / 复制 / 预测回显`；提示词禁止把可执行命令散落在正文，教学链路仍维持学生单步确认。
- [x] **教学命令卡事件配对**（2026-09-10，DEV-JOURNAL §37.140）：教学专用工具与 shell 映射工具均发送 `started → completed`，前端可可靠生成一张卡；教学卡不再展示“只读探测，无副作用”等影响标签。
- [x] **教学卡简化与复合回显关联**（2026-09-10，DEV-JOURNAL §37.141）：教学工具不再显示重复 JSON Input；复合命令仅接受本次 Agent 注入首段完成块，避免卡片超时后锁住下一步。
- [x] **全局打字机响应与速度调校**（2026-09-10，DEV-JOURNAL §37.142）：Run 后首字符等待缩短，逐字节奏全局放慢；教学模式不设独立速度。
- [x] **Run 首字符延迟与全局节奏二次修正**（2026-09-10，DEV-JOURNAL §37.143）：移除清行后的固定等待，恢复正常逐字节奏；所有模式继续共用全局速度。
- [x] **打字机清行补回车**（2026-09-10，DEV-JOURNAL §37.144）：`Ctrl-C` 后发送回车，避免命令粘在 `^C` 后面。
- [x] **打字机回显即时转发**（2026-09-10，DEV-JOURNAL §37.145）：AgentCommandEcho 不再缓存整条命令，PTY/SSH 回显字符到达即显示并保留蓝色标记；错配与命令完成后的普通输出均正确复位。
- [x] **Run 注入不污染 OSC 7**（2026-09-11，DEV-JOURNAL §37.146）：蓝色回显匹配器跳过 OSC/CSI 等控制序列，避免 `file://localhost/...` cwd 标记泄漏到终端文本。
- [x] **教学卡兼容渲染与提示符着色边界**（2026-09-11，DEV-JOURNAL §37.147）：Teach 模式兼容旧式加粗/纯文本板块标题并恢复概念、示例、易错、练习分区；人类打字/教学 Run 等待 shell 提示符边界，避免 `[root@server ~]#` 被命令蓝色误染。

## 五、下一步（按优先级）

1. **P0 原生稳定性收口**：由用户在 Tauri 桌面端分别验证可见终端蓝色命令、慢速打字机、动态 `dnf` 回显、提问卡暂停、教学单步链，以及第二台 SSH 服务器的资源树跟随/上传。任何问题先以对应会话日志和工具调用 ID 定位，不以浏览器 mock 判断。
2. **P0 工具生命周期统一**：补齐所有工具的 `tool_call_id`、started/chunk/completed/error/cancelled 关联与跨 session 隔离测试；在此之前维持串行执行，避免长任务的乱序与虚假结果。
3. **P1 桌面级回归自动化**：建立最小原生 Tauri + SSH 场景，而非修复已过期的 Playwright mock；覆盖 SSH 输出、审批 FIFO、文件写入和教学命令回传。
4. ~~**宣传片（外部制作）**~~ ✅ 用户 2026-09-11 已自行完成；`promo-video/` 工程与其截图素材经确认弃用并删除（`.gitignore` 防回流），无需再维护录屏/动画工具链。
