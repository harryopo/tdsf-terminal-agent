...
| 39 | **UI 三修**（2026-08-31 用户反馈，细节见 DEV-JOURNAL §37.87）：①BackendPill Strands 弹窗改 `bg-popover` 跟随明暗主题（根因=shadcn tooltip 默认反色 bg-foreground，深色主题下白底）+ 描述精简为标题+副行（去 rust_bridge_active 等调试串）；②AgentModeSwitcher 从四档横排+底部说明行改为**折叠面板**（触发按钮显示当前模式，点击向上弹每档=图标+名称+描述，点外/Esc 关闭）；③WorkspaceEnvSelector 删 SSH 齿轮 + 删 Refresh（打开菜单自动刷新 WSL 列表补功能）+ 三条目间距统一 | ✅ 已完成 | 2026-08-31；commit a990289；门禁：tsc/lint 0 + vitest 1205（切换器 12 项重写全绿；SnippetsPanel 1 例 flaky 与本次无关）+ build:web；**待用户桌面实测**：Strands 悬浮弹窗黑底 / 模式折叠面板交互 / 工作区菜单三项 |
