...
| 42 | **SSH 稳定性检查收尾（§37.90 检查报告 4 缺陷修复）**：P1 重连后 SFTP 缓存+隧道资源失效修复（perform_reconnect 加 on_reconnected 回调 → SshState.invalidate_session_resources：remove_sftp+stop_tunnels_for_session）；P2 agent 会话枚举（ssh_status 接 sidecar 反向路由 + ssh_list_sessions readonly 工具 + host 校验放宽到任一连接中会话）；P3 顺手修（ssh_command.py 过时注释 / /tmp 注入脚本 trap 清理）；P3 调研后做（断连事件通知 sidecar，先查 Rust→Python 通道）；P3 可选暂缓（Failed 后手动恢复入口） | ⏳ 待启动 | 方案+排期+接手指南：`docs/SSH连接稳定性检查报告与下一步规划-2026-08-31.md`（§37.90）；P1 真实稳定性隐患优先 |
