...
> **最后更新**：2026-09-11 · **§37.147 教学卡兼容渲染与提示符着色边界**（§37.142—§37.147 为打字机节奏与蓝色命令回显连续调校，均待原生复测）。交接先读 **§37.101 会话交接章** 与 §37.124—§37.147（真实稳定性 W0-W5、教学模式终端化、SSH 链路收口）。
>
> **2026-09-11 仓库整洁**：宣传片工程 `promo-video/` 经用户确认弃用并删除（未入库）；`.workbuddy/`（第三方飞书画板工作区）与 `diagrams/`（其架构图产物）、根目录 `agent-logs/` 均加入 `.gitignore` 不入库。用户已自行完成服务器连接与 SSH 使用验收；回答置信度（T10.1 引用协议）列入后续优化。
...
**修复与验证**：成功现在仅限明确的整数 `exit_code == 0`；非零退出码和缺失/非法退出码都返回 `error`，审计为 `command_failed`，证据为 error。新增两条回归；受影响的 `test_tools.py` **185 passed**，Python `py_compile` 通过。

**下一步**：这只解决一次调用的结果真实性。W3 仍需在审批前创建 SQLite operation，在派发前后原子记录状态，并在 sidecar 重启后把 `dispatching/dispatched` 变为 `indeterminate`，禁止自动重发可能写入远端的命令。
...
**执行波次**：W0 原生 Tauri/SSH 证据；W1 M1 family 过滤与展示；W2 成功执行历史和教学结果协议；W3 operation/intent durable execution（重启后写操作不自动重放）；W4 回答主张到来源绑定与确定性输出安全；M3/M4 功能只在这些边界稳定后推进。
...
**实现**：`ToolCallLimitHook` 现在把除 `ssh_command`（其直连执行路径已单独记录）外的真实完成工具调用写入 `EvidenceTracker`，因此知识库检索/全文读取也有可追溯事件。新增 `evidence.assess(session_id)`：只计入成功完成的来源工具；无来源为“待核验”，有来源为“已有依据”，写操作后又有只读验证才为“操作已验证”。教学模式只在最新助手消息下显示这一状态及实际工具名；RPC 不可用时保守显示“待核验”，绝不回退到文本关键词猜测。

**验证与边界**：新增证据状态 pytest 4/4 通过，相关 hook 定向用例通过；全量 Vitest **1339**、typecheck、lint、cargo check 均通过。合并 Python 定向运行是 32/33：唯一失败为既有 SSH 集成用例在当前环境缺少 `langgraph`，确认模式按 fail-closed 拒绝命令；没有为凑绿放宽安全策略。该状态只说明本会话有真实工具事件，**不证明该段回答逐句为真**。如产品仍需“回答置信度”，下一步必须先设计主张与知识库片段/工具输出的显式引用协议，再决定 DSPCR5 的适用位置。
...
- §37.91 **SSH 稳定性 #42 P1 修复落地 + P3 顺手修**（commit 775d82f，4 文件 +262/-28）：**P1 重连资源失效**——`SshReconnectConfig.on_reconnected` 回调（perform_reconnect 热替换后、connected 广播**前** await，时序保证前端重连后的 SFTP 不命中失效缓存）→ `ssh_connect` 经 `AppHandle::state::<SshState>()` 注入（SshState 非 Arc，最小侵入免重构）→ `SshState.invalidate_session_resources`：remove_sftp（spawn close）+ stop_tunnels_for_session；回调不持会话锁（锁三不变量）、幂等容忍 close 竞态。**P3-3**——脚本组装抽纯函数 `build_integration_commands`，bash/zsh EXIT trap + fish fish_exit 清理行写入 heredoc 内容内、先于 const（覆盖 early-return 路径），修 /tmp/tdsf-osc7-* 无限累积。**P3-2**——ssh_command.py docstring 对齐 P0-D 现状。**顺手**——human_type.rs clippy 8 警告清零（B2 遗留）。门禁：cargo test **360**（+6）全绿 / clippy --all-targets **0** / pytest rust_bridge+modes 69；**待用户 tauri:dev 实测**（连 SSH→SFTP→断网重连→SFTP 立即可用 + /tmp 无累积）。详 DEV-JOURNAL §37.91
- §37.90 **SSH 连接稳定性系统性检查（无代码改动，纯检查+规划）**：Rust 后端 ssh/ 全 8 文件通读 + agent 集成链路探查。结论：**设计/心跳/重连均良好**（keepalive 15s×3+300s 原生配置、自动重连指数退避 6 次、TOFU 5min fail-closed、PTY/SFTP 解耦），**发现 4 缺陷**：P1 重连后 SFTP 缓存+隧道资源不失效（perform_reconnect 只换连接，SshState.sftp_sessions 仍挂旧 channel；真实稳定性隐患）；P2 agent 无会话枚举（ssh_status 未接 sidecar 反向路由，多主机运维不可用）；P3×3（断连不通知 sidecar / ssh_command.py 注释过时 / /tmp 注入脚本不清理）。修复方案+排期+接手指南已归档 `docs/SSH连接稳定性检查报告与下一步规划-2026-08-31.md`（含逐项改动文件/门禁/防踩坑）
- §37.89 **工作区逻辑系统修复**（用户实测七问题，7 前端文件）：①SSH 连接**只绑专属 SSH Space**（host/user 匹配），绝不改写本地/WSL Space（旧逻辑把"当前 Space"整体 setEnv(ssh)，是"本地工作区被导向服务器/WSL 显示服务器 IP"根因）；②`autoConnect` 标记从 store 级（有竞态）改为会话创建时写入 `session.autoConnect`——开机自动连接无匹配 Space 直接跳过+断孤儿会话，手动连接无匹配才新建；③删 Space 即断其 SSH 连接（无共用时 disconnect）；④断线/启动降级 `setEnv(local)+setRoot(null)` 成对清理（修 `[InvalidPath]`，useSpaces 新增 setRoot）；⑤新建本地 Space 的 defaultEnv/defaultRoot 强制本地（SSH 时降级 LOCAL_WORKSPACE）；⑥SSH tab 标题统一 "shell"；⑦顶栏横向工作区标签（SpaceSwitcher 每 Space 一 chip，点激活开总览）；⑧+ 菜单/命令面板/快捷键删 Blocks/Privacy/Preview/Git Graph（只留 Terminal/Editor）。门禁 tsc/lint/vitest **1268**/build 四绿；**待 tauri:dev 桌面实测**；交接 `docs/工作区逻辑修复-交接说明-2026-08-31.md`
- §37.87 **UI 三修两轮完成**（commits a990289 + e7cde62，含 4 文件被并行对话 af9e9a3 卷走一半）：一轮=Strands tooltip 主题化（bg-popover 黑配黑）+描述精简、AgentModeSwitcher 折叠面板、工作区菜单删齿轮/Refresh；二轮=模式显示去重（删 AiComposerInput 并列 Pill）、弹层 Portal+fixed 根治遮盖、**假 agents_count 移除**（sidecar.health 该字段来自 LangGraph fallback AGENT_REGISTRY，与 Strands 无关，用户质疑属实）、tooltip flex-col 修基线、模式卡片复刻用户参考图（✓勾+图标+名称+右侧 brief 简短说明，registry META 加 brief 字段）、StatusBar [Agent 模式][Strands] 位置互换；门禁 tsc/lint 0 + vitest 117 files 全过 + build:web；**待桌面实测：模式抽屉卡片 / Strands 弹窗黑底两行纵排 / 状态栏顺序**
- §37.85+86 **知识库全链路治理完成 + 翻译交接外部 AI**（commit fb0d76d，5 文件 +985）：①**根因修复**——爬虫 HTML 解析改 markdownify 转标准 GFM（表格/列表/代码围栏全保留，html2text 实测表格缺前导 | 弃用）；②**垃圾治理**——clean_title/is_junk_title/is_link_farm/_dedupe 四重过滤（137 垃圾页），重爬 682 条；③**大整合**——686 文件 → 27 合并 md（7 分类 × ≤5，CONSOLIDATED_DOCS fail-closed 映射），块级重建 4647 块（每块 ≤1200 字嵌入完整）；④**全量清洗**——删 25 垃圾章节（Gentoo Wiki 12 个语言翻译页——根因 URL 末段语言码 `wiki/FAQ/de` 补漏 + Apache 旧站 11 Keywords 开发向页 + 2 版本索引页）+ 42+ 内容残渣行（See also 堆/纯链接/模板导航），字符 -5.3%；⑤**翻译交接**——`docs/知识库中文翻译-交接说明.md`（完整任务书：状态盘点/脚本用法/成本 A ¥11.6/22.7h B ¥7.5/14h C ¥0.1/9min/格式要求/踩坑清单），翻译脚本 translate_knowledge.py 断点续跑就绪，**content_zh=0 待外部 AI 执行（范围先问用户）**；门禁 pytest **1881** / vitest **321** / tsc / lint 全绿；**待外部 AI：翻译 → 重导导出 → 用户桌面实测**
- §37.84 **知识库 6+1 分类 + LLM 全量中文翻译 + Linux 哲学专属分类完成**（commit ffddb47，21 文件 +3391）：①**分类**——`sources.category_for(source, title)` 17 官方源一源一主分类（archwiki 按 title 关键词分流 sys-admin/basic-ops），category key 英文（basic-ops/cmd-tools/sys-admin/net-remote/security/services/linux-philosophy）；rag entries 加 category+content_zh 列（ALTER 幂等）、fts_entries 加 content_zh_tokens（虚拟表 DROP 重建回填），list/search/get/list_files 全链路带 category + list_files(group) 过滤；②**繁体丢弃**（用户钦定推翻 §37.83 C1）——clean.py 删 to_simplified，generic.py 两处整条丢弃计 discarded_traditional；③**translate_knowledge.py**——断点续跑/小条目合批（===zh-sep-N=== 切回）/长度校验 30%-300% 重试 2 次/每 10 条进度，deepseek 后台长跑中（619 条 585 批 ~10 小时量级，中文 query 命中译文已端到端验证 match_type=both）；④**philosophy/** 4 篇随源码分发教学语料（Linux 哲学/命令词源对照/命令设计/目录逻辑，从 corpus_personal 12 文件清洗重组剔除个人课程内容，106 块）+ load_philosophy_docs 启动/重建幂等入库（第 7 分类 linux-philosophy）；⑤**前端** KnowledgeBrowser 按 category 6+1 分组（中文组头+来源中文副行+未分类归其他沉底）+ 导出 `knowledge-preview/<分类>/<源>/<标题>.md` 两级层级含中文译文段；重建后 621 条官方（繁体 2 条丢弃）+ philosophy 106 块，分类分布 cmd-tools 200/services 256/security 82/net-remote 43/sys-admin 33/basic-ops 7；门禁 pytest **1751** / vitest **24** / tsc / lint / build:web 全绿；**待实测：tauri:dev 知识库 6+1 分组显示 + 翻译完成后中文搜索**
- §37.81 **v3.1 三模式信任体系与教学特色完成**（2026-08-29，spec `add-agent-trust-modes`，commit c671fef，69 文件 +8234）：①**三模式**（observe/confirm/auto，`modes.py` + `decide(risk,mode)` 矩阵 + observe schema 级隔离 + 缺省 confirm 降级）+ **BREAKING 删 4 子 agent 委派**（`_SUB_AGENT_SPECS`/agent-as-tool/双缓存删除，工具集 24→20，前端 5 入口收敛 main + AgentModeSwitcher 三档 + Teach 开关 + Pill 改模式指示，per-session 持久化）；②**审批卡四层**（语义/命令原文/LLM 解释/影响预测 L0-L4 色带，`command_impact.py` 复合命令拆解 + denylist 硬底线 + 危险构造检测）+ 三按钮（拒绝附言/⚡本会话只读免审/执行）+ **双轨反馈**（用户拒绝可协商 vs command_blocked 如实报告）+ host 校验 + 5min 超时 fail-closed + `NeedsYouApprovalCards` 接线闭环（needs_you 事件原本前端零消费，新建订阅渲染）；③**免确认记忆三级**（`trust_store.py`：会话级⚡免审+前缀免批内存态 / 白名单 allow-ask-deny 持久化+设置 UI / deny 硬底线任何模式不可绕，评估顺序有单测锁定）；④**终端感知**（SSH 注入补 OSC133 全套+633;E/P 幂等保序 + 前端 block 流水账 author=agent + `<environment>`/`<terminal-history>` XML 分区注入，`system.probe_env` 会话级缓存）；⑤**打字机**（`human_type.rs` Weibull 算法 + `pty_write_human`/`ssh_write_human` 双命令 + 0.2-5× 滑杆 + 用户按键打断 + sudo 降级整段 + >200 字符自动整段）；门禁：pytest **1651** / vitest **1187** / cargo test **407** / tsc / lint / build:web / cargo check 全绿 + tauri:dev sidecar ready 11.9s 114 方法注册；**待用户实测：方案书 v3.1 §7 验收 1-8（三模式/审批卡/白名单/终端感知/打字机）+ 红线 9 回归**
- §37.80 **P3-T14 会话记忆沉淀完成**（方案书 v3.0，与 B3 /summary-to-skill 合并设计）：①**sidecar `session_memory.py`**——summarize_session（LLM 生成"现象→根因→解法→教学要点"摘要，幂等 ID `session-memory-<session_id>` 写统一 RAG，source=session-memory，重复沉淀自动覆盖）+ save_session_skill（写用户目录 SKILL.md + skills.registry 热重载立即生效）+ memory.summarize_session / memory.save_skill 双 RPC 注册；②**agent 工具 save_skill** 入 TOOL_REGISTRY（registry 驱动 fail-closed 门禁继承）；③**前端 chatStore 收尾钩子**——newSession/deleteSession 切走时 maybeSummarizeSession 自动摘要（≥3 条消息才触发，会话级去重）；④**transport.ts 首轮记忆注入**——新会话首条用户消息检索 knowledge.search（source 过滤 session-memory/session-case，top3）生成 `<session-memory>` 块注入，Promise.race 3s 超时静默跳过不阻塞首响；**待实测：真实 LLM 多轮对话 → 切换会话触发摘要 → 新会话首条消息看记忆注入 → agent 调 save_skill 沉淀**
...
### 37.37 翻译卡片智能翻转定位（2026-08-09 ✅ 完成）

**现象（用户反馈）**：底端命令行划词翻译，卡片出现在底端显示不全被遮住；要求"底部划词时卡片出现在词的上方"。

**根因**：`TranslateTooltip.tsx` 定位固定 `top = y + 12`（永远在选中点下方），底部划词时卡片超出视口被窗口边缘遮住。
...
- #20 服务器实时监控仪表盘（✅ 已完成：commit 7602c73 落地，详见 §37.52）
...
### 37.52 服务器实时监控仪表盘（ROADMAP #20）确认完成 + 文档滞后修正（2026-08-11 ✅）

**背景**：用户指出"仪表盘不是做完了吗"——核实确认 **#20 已在 commit 7602c73（2026-08-11 10:07，审计 P0-P2 修复）首次创建并完整落地**，但当时 ROADMAP/dev-state 多处仍标"📋 待启动"，属文档未同步。本次仅修正文档，无代码改动。

**功能全貌（`src/modules/server-monitor/`，App.tsx `ServerMonitorEntry` 集成于侧栏）**：
- **SSH 免 Agent 采集**：`useServerMetrics.ts` 通过 `sshCommand` 执行合并命令（`COLLECT_CMD` 一次往返采集 /proc/stat + free + df + /proc/net/dev + ps），概览命令（hostname/os-release/uname/uptime/loadavg）仅首次连接执行一次
- **3s 轮询**：`DEFAULT_INTERVAL = 3000`；CPU/网络差值法（保存上一次快照）；历史保留 60 点
- **5 模块**：CPU（iowait 对齐 top/htop）/ 内存 / 磁盘 / 网络 / 进程 Top N（ps aux --sort=-%cpu | head -6）
- **自动停止**：面板关闭或会话断开停止轮询；连续失败 3 次（MAX_FAILURES）自动停止并标记 error
- **健壮性（审计 P1 #5-9 同批落地）**：isCollectingRef 并发锁 / MonitorErrorBoundary / collectOverview catch 日志 / coerceServerMonitorInterval 白名单 / 28 个 parser 测试

**文档修正清单**：
- `docs/ROADMAP.md`：#20 → ✅ 已完成（注明 7602c73 + 功能摘要 + §37.52）
- `docs/dev-state.md`：§37.48/37.49/37.50/37.51 四章"接手下一步"中的 #20"待启动"行 → ✅ 已完成注记；§37.49 的隧道"待启动"行 → ✅（§37.50 已覆盖）
...
### 37.137 宣传片交接与当前开发主线（2026-09-10）

**交接状态**：用户决定宣传片改由其他 AI 完成。`promo-video/docs/production-spec.md` 已冻结最终字幕、约 100–112 秒分镜与“只交付带 BGM”规则；`promo-video/HANDOFF.md` 记录真实截图、缺失动态录屏、UI 不得伪造及产品事实。由于当前桌面自动化通道没有枚举到 TDSF 原生窗口，本轮不能录制真实操作，未以伪造界面代替。

> **2026-09-11 更新**：用户已自行完成宣传片，本目录（含上述 spec/handoff 与全部截图素材）确认弃用并已从工作区删除；`.gitignore` 保留 `promo-video/` 条目不使其回流。本节仅作历史记录，相关动画/录屏工具链不再维护。

**当前主线**：视频不再占用开发优先级。下一手应先按真实桌面会话复测 §37.132–§37.136 的 SSH/可见终端/提问/教学/资源树链路；随后统一工具 `tool_call_id` 与完整生命周期事件并保持串行，最后建设小范围的 Tauri+SSH 原生回归。浏览器 mock 仅保留单元回归价值，不能作为产品验收。

### 37.138 教学命令卡 Schema 兜底（2026-09-10 ✅）
...
### 37.144 打字机清行补回车（2026-09-10 ✅，待用户原生复测）

清行序列从单独的 `Ctrl-C` 改为 `Ctrl-C + 回车`（`\x03\r`），避免 shell 将下一条 Agent 命令接在 `^C` 后面。清理完成后仍立即开始输入，不恢复固定等待；所有模式继续共用全局 `agentTypingSpeed`。重启桌面端后验证已有残留输入和空提示符两种 Run 场景。
