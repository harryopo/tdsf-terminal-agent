...
> **最后更新**：2026-08-30 · **知识库 6+1 分类 + LLM 全量中文翻译 + Linux 哲学专属分类完成**（§37.84，commit ffddb47：category_for 17 源映射 + 繁体丢弃（推翻 §37.83 C1 转简）+ content_zh 译文列/FTS 双写 + translate_knowledge.py 断点合批翻译 + philosophy/ 4 篇随源码教学语料 + 前端 6+1 分类分组 + 导出分类两级层级）。接手请直接看 **DEV-JOURNAL §37.84**。**翻译后台长跑中（619 条 585 批，~10 小时量级，断点可续跑），完成后中文 query 直接命中译文**。下一步 = 翻译完成后重导 knowledge-preview + 用户桌面实测知识库 6+1 分组 → T13（MCP 客户端）
