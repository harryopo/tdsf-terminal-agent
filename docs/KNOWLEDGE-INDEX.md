...
| 文档 | 路径 | 作用 | 当前版本 |
|------|------|------|----------|
| **当前状态/进度/已知问题** | `docs/dev-state.md` | ⭐**唯一进度记忆源**：§一到§37.24交接章，接手看末尾「§37.24 交接指南」 | §37.24（2026-08-04） |
| **架构现状澄清** | `docs/dev-state.md` §37.16 | 9-Agent 未在主路径集成的事实 + 规划 | 必读 |
| **短/长期规划** | `docs/ROADMAP.md` | 方案书 P0-P4 路线图跟踪 + 下一步清单 + 待决策项（收尾更新） | 必读 |
| **开发日志** | `docs/DEV-JOURNAL.md` | 任务/方案/报错与修改（根因+解法）/复盘，经验沉淀防重复踩坑（收尾追加） | 必读 |
...
- §37.14-37.16：**交互重构（删 SSH 面板/全删 Space/欢迎界面内嵌）+ 架构现状澄清（9-Agent 未集成）**
- §37.17：**方案书 v1.0 定稿**（Agent 架构拍板 B 方案：Strands 多 agent 真集成）
- §37.18：**P0-6 Agent 全链路打通**（main 统一入口 + 自主委派 Agents-as-Tools + 可视化）
- §37.19：**P1 可信与安全三件套**（HITL 真实审批闭环 + 会话证据链 + hash 审计链）
- §37.20-37.21：**P2 翻译模块重构 + 知识库可视化**
- §37.22-37.23：**全量工程 T1-T5 + sidecar onedir 打包发布闭环 + 黑屏根因修复**
- §37.24：**dev 启动黑屏排查 + 交接基线**（2026-08-04，P0-P4 全部完成）
...
4. 读 `docs/dev-state.md` 末尾「§37.24 交接指南」
5. 读本索引了解文档全貌
...
### 3.3 关键 commit 节点（2026-07-30 ~ 2026-08-04）
...
**2026-08-01 ~ 2026-08-04（§37.14-37.24 全量工程）**：

| commit | 内容 |
|--------|------|
| `5450309` | dev-state §37.24 交接基线（dev 启动黑屏排查教训） |
| `7cb230d` | **sidecar onedir 打包发布闭环 + 黑屏根因修复**（NSIS 402MB） |
| `69ec9c0` | **P4 单框架收敛**：删除 LangGraph 遗产（2184 行 + 5 依赖） |
| `b22051b` | **P2-4 决策库**：AI 排障成功自动沉淀案例 |
| `5a78456` | **T3 词库增强**：ECDICT 8.1 万条 + lemma 词形还原 |
| `b59b3a8` | **T2 透明窗口修复**：渲染前即不透明 + 首帧后 show |
| `c76be91` | **T1 简洁终端图标**：修复纯蓝紫单色占位 |
| `a2aa150` | **P2 翻译模块重构**：统一选中浮层（翻译 + Ask TDSF） |
| `f35659f` | **P1-3 hash 审计链**：sha256 前后链 JSONL |
| `4cc840e` | **P1-2 会话证据链**：EvidenceTracker |
| `139fc21` | **P1-1 HITL 真实审批闭环**：threading.Event 等待-唤醒 |
| `31fa409` | **交互重构**：删 SSH 面板 + Space 全删 + 欢迎界面内嵌 |
...
> **最后更新**：2026-08-04 · v1.3 · 同步 §37.24 交接基线（P0-P4 全量工程完成 + sidecar 打包发布闭环）。上游参考：https://github.com/crynta/terax-ai
